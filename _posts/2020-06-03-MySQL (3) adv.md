...
```
- 普通索引A、普通索引B，使用了普通索引A
- 按序建立的普通索引A、普通索引B、联合索引(A,B)，使用了普通索引A
- 按序建立的联合索引(A,B)、普通索引A、普通索引B，使用了联合索引(A,B)
...
例如索引`A,B,C`，使用索引排序的为
```
    WHERE ... ORDER BY A='a' and B='b'
    WHERE A='a' ORDER BY B
```
未使用索引排序的为
```
    # 引用了一个不在索引的列
    WHERE ... ORDER BY A='a' and B='b' and D='d'
    # 第二列是范围
    WHERE A='a' and B IN ('b', 'bb') ORDER BY C='c'
```
#### 延迟关联
